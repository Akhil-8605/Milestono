...
const MobGalleryImage = require("../models/mobGalleryImage");
...
const createGalleryImage = async (req, res) => {
  try {
    ...
    const newImage = new MobGalleryImage({
      title,
      image : URL,
    });
...
const getGalleryImages = async (req, res) => {
  try {
    const images = await MobGalleryImage.find();
...
const updateGalleryImage = async (req, res) => {
  try {
    ...
    const updatedImage = await MobGalleryImage.findByIdAndUpdate(
      id,
...
const deleteGalleryImage = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedImage = await MobGalleryImage.findByIdAndDelete(id);
